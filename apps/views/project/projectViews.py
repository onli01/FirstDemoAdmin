...
class ProjectView(object):
  ...
  @staticmethod
  async def addProject(name,owner,**kwargs):
    """
    :param name: 项目名
    :param owner: 所属人
    :return:
    """
    try:
      user =await Project.query.filter_by(name = name).first()
      if user is not None:
  ...
  @staticmethod 
  async def getProjectByName(name):
    try:
      project =await Project.query.filter_by(name = name).all()
      if project :
        return project,None 
      else:
        return None,'项目不存在！'
  ...
  @staticmethod 
  async def getProjectAll():
    try:
      project =await Project.query.filter_by(status=1).all()
      return project,None 
    except Exception as e:
